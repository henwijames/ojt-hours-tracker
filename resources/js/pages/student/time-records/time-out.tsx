...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Time Out',
        href: '/student/time-records/time-out',
    },
];
                ...
                router.post(route('student.face-recognition.compare'), faceData, {
                    onSuccess: (page) => {
                        ...
                        if (flash.type === 'success') {
                            // If face matches, capture image and submit time out
                                ...
                                if (imageSrc) {
                                    ...
                                    fetch(imageSrc)
                                        ...
                                        .then((blob) => {
                                            ...
                                            formData.append('image', blob, 'time-out.jpg');

                                            router.post(route('student.time-records.time-out.store'), formData, {
                                                onSuccess: () => {
                                                    router.visit(route('student.time-records.index'));
                                                },
                                                onError: (errors) => {
                                                    toast.error(errors.message || 'Failed to record time out');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="from-background to-muted/20 @container/main flex flex-1 flex-col gap-6 bg-gradient-to-br p-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Time Out Verification</CardTitle>
                    </CardHeader>
